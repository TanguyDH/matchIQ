...
function NewStrategyMenu({ onNew, onGallery }: { onNew: () => void; onGallery: () => void }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((s) => !s)}
        className="bg-[#10b981] hover:bg-[#34d399] text-[#0f172a] text-xs font-semibold px-4 py-2 rounded-lg transition-all hover:shadow-[0_0_20px_rgba(16,185,129,0.35)] flex items-center gap-1.5"
      >
        + New
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div className="absolute right-0 top-9 z-20 w-48 bg-[#1e293b] border border-[#334155] rounded-lg shadow-[0_8px_24px_rgba(0,0,0,0.5)] py-0.5">
            <button
              onClick={() => { onNew(); setOpen(false); }}
              className="w-full text-left px-3 py-2 text-xs text-[#94a3b8] hover:bg-[#334155] hover:text-[#f1f5f9] transition-colors"
            >
              <span className="mr-2">✦</span>New strategy
            </button>
            <button
              onClick={() => { onGallery(); setOpen(false); }}
              className="w-full text-left px-3 py-2 text-xs text-[#94a3b8] hover:bg-[#334155] hover:text-[#f1f5f9] transition-colors"
            >
              <span className="mr-2">◈</span>Import from gallery
            </button>
          </div>
        </>
      )}
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <NewStrategyMenu
          onNew={() => router.push('/strategies/create')}
          onGallery={() => router.push('/strategies/gallery')}
        />
      </div>
