...
function OutcomeDropdown({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return DESIRED_OUTCOMES.filter(
      (o) => o.label.toLowerCase().includes(q) || o.group.toLowerCase().includes(q),
    );
  }, [search]);

  const grouped = useMemo(() => {
    const map: Record<string, typeof DESIRED_OUTCOMES> = {};
    for (const o of filtered) (map[o.group] ??= []).push(o);
    return Object.entries(map);
  }, [filtered]);

  const selectedLabel = DESIRED_OUTCOMES.find((o) => o.value === value)?.label;
